...
export default async function EleminationRoundMatch() {
  ...
  const isCompleted = resp?.length > 0 && resp[0].matchSchedule.length > 0 && resp[0].matchSchedule.every((val)=>val.matches.every(dat=> dat.winner !== "TBA" ));
  ...
  if (isCompleted && elimationResp.length > 0) {
    return (
