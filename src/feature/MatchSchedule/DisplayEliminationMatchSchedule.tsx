...
export default function DisplayEliminationMatchSchedule(props: Props) {
    ...
    const filterSemifinal = games[0].matches.filter((val) => val.matchType === MatchType.SEMIFINAL.toString());
    
    const final = games[0].matches.filter((val) => val.matchType === MatchType.FINAL.toString());
    console.log(final);
  return (
    <div className=" mx-auto w-full">
      <h1 className="text-2xl font-bold mb-4">Elimination Round Matches</h1>
      <p className="text-gray-600 mb-6">All matches have been completed.</p>
      <div className=" flex flex-row w-full h-[300px]">
        <div className=" flex flex-1 flex-col relative border-t-1 border-t-black border-r-1 border-r-black border-b-1 border-b-black border-r-1 border-r-black ">
            <div className="   absolute -bottom-7 ">
                      <MatchCard isElimination  isUseMatchScore={true} data={filterSemifinal[0]} id={filterSemifinal[0].id} games={data}/>
          </div>
