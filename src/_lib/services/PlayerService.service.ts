...
export async function getPlayerTeamId(teamId: string): Promise<Array<Player>> {
  try {
    ...
    const resp = (await getData(payload)) as Array<Player>
...
export async function getAllPlayer() {
  try {
    ...
    const resp = (await getData(payload)) as unknown as Array<Player>
    const teams = await getAllTeams()
    const playerWithTeam = resp.map((val) => {
      const teamData = teams?.find((team) => team.id === val.teamId)
...
export async function insertPlayerViaBatchUpload(
  batchData: InsertPlayerViaBatchUpload[]
) {
  try {
    const insertPromises = batchData.map(
      async (val: InsertPlayerViaBatchUpload) => {
        ...
        return await insertPlayer(payload)
      }
    )
...
export async function updatePlayer(
  ...
  try {
    ...
    await updateDoc(docRef, playerData as any)
  } catch (error) {
