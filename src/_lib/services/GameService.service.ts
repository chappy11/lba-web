...
export async function insertGame(
	...
	try {
		...
		const teamOnePlayers: Array<Player> =
			await getPlayerTeamId(
				payload?.teamOne?.id as string
			);

		const teamTwoPlayers: Array<Player> =
			await getPlayerTeamId(
				payload?.teamTwo?.id as string
			);

		const teamOnePlayerRecord: GamePlayer[] =
			teamOnePlayers.map((val) => {
				return {
		...
		const teamTwoPlayerRecord: GamePlayer[] =
			teamTwoPlayers.map((val) => {
				return {
					playerId: val?.id as string,
					firstname: val.firstname,
					middlename: val.middlename,
...
export const getGetGamesById = async (
	gameId: string
) => {
	try {
		const docRef = doc(
